...
- 展开态：Web 版改为 notch-free 小浮层，不再从顶部硬件 notch 向下展开；桌面约 328×116px，移动端约 260×104px，固定在导航栏下方右侧，避免遮挡移动端导航。
...
| notch-free 移动端几何 | 390×844、DPR 2：nav bottom 30px，expanded top 42px、right 380px、size 260×104px，未覆盖 nav；打开 mobile menu 时命中元素为菜单链接 |
| notch-free 桌面几何 | 1440×1000、DPR 2：expanded top 40px、right 1416px、size 328×116px，未覆盖 nav |
...
追加检查结果：Web 展开态改为导航栏下方的右侧浮层，并将展开态 z-index 降到导航之下。390px 移动视口中 expanded notch 与导航栏没有几何重叠；mobile menu 打开时与浮层有视觉区域重叠，但菜单位于更高层级，命中测试落在菜单链接上，不会被浮层阻挡。桌面展开态同样脱离顶部 notch，尺寸缩小到 328×116px。
